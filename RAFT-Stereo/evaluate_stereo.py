...
def validate_things(model, iters=32, mixed_prec=False):
    ...
    model.eval()
    val_dataset = datasets.SceneFlowDatasets(root='/media/sde/datasets/SceneFlow', dstype='frames_finalpass', things_test=True)
    ...
    for val_id in tqdm(range(len(val_dataset))):
        ...
        # image1 = image1[:, :, start_h:start_h + crop_h, start_w:start_w + crop_w]
        # image2 = image2[:, :, start_h:start_h + crop_h, start_w:start_w + crop_w]
        # flow_gt = flow_gt[:, start_h:start_h + crop_h, start_w:start_w + crop_w]
        # flow_gt = flow_gt.abs()
        # valid_gt = valid_gt[start_h:start_h + crop_h, start_w:start_w + crop_w]
    ...
    mask = ~np.isnan(epe_list)
    epe_list = epe_list[mask]
    epe = np.mean(epe_list)
    d1 = 100 * np.mean(out_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_ckpt', help="restore checkpoint", default='./models/raftstereo-sceneflow.pth')
