...
def fetch_optimizer(args, model, total_steps):
    """ Create the optimizer and learning rate scheduler """
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.wdecay, eps=1e-8)

    scheduler = optim.lr_scheduler.OneCycleLR(optimizer, args.lr, total_steps + 100,
...
def train(args):
    ...
    total_steps = int(args.epoch * len(train_loader) / args.batch_size)
    ...
    for epoch in range(0, args.epoch):
        ...
        save_path = Path('models/realtime/%d_%s.pth' % (epoch + 1, args.name))
        ...
        if epoch % 2 == 0:
            ...
            torch.save(checkpoint, 'models/realtime/checkpoint/ckpt_%d_%s.pth' % (epoch + 1, args.name))
    ...
    logger.close()
    PATH = 'models/realtime/%s.pth' % args.name
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', type=int, default=20, help="length of training schedule.")
    parser.add_argument('--image_size', type=int, nargs='+', default=[544, 768],
