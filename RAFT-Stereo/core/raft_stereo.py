...
class RAFTStereo(nn.Module):
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False):
        ...
        with autocast(enabled=self.args.mixed_precision):
            ...
            net_list = [torch.tanh(x[0]) for x in cnet_list] # x[0]: hidden
            inp_list = [torch.relu(x[1]) for x in cnet_list] # x[1]: context
        ...
        for itr in range(iters):
            ...
            with autocast(enabled=self.args.mixed_precision):
                ...
                if self.args.n_gru_layers >= 2 and self.args.slow_fast_gru:# Update low-res GRU and mid-res GRU
                    net_lw_scalesist = self.update_block(net_list, inp_list, iter32=self.args.n_gru_layers==3, iter16=True, iter08=False, update=False)
